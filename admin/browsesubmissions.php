   ...
   <head>
	...
	<div  class='DivOne'>
		...
		<div data-role="navbar">
			<ul>
				<li><a href="browseguides.php" data-ajax="false">Back</a></li>
	...
	<?php
	/*function that takes in a guide name and queries a table in the database for the particular guide and prints out  a list all
	submissions made through the guide and saved in the database*/
			
	function viewAns($gd){
		include 'scripts/dbCon.php';			//script for database connection
	
			$result = mysqli_query($dbCon,"SELECT * FROM guides t,submission s	 WHERE s.guidename = t.name and s.guidename=$gd"); 		//query table for guide submissionss
			...
			/*	this loop prints out the submissions available for a particular guide from the database with their respective 
			...
			while($row = mysqli_fetch_array($result))
			{
				...
				echo "<td><form method='POST' target='_blank' data-ajax='false' action='viewsubmission.php'>";
				...
				echo "<td><form method='POST' data-ajax='false' target='_blank' onsubmit='return confirm(".$yy.");' action='scripts/deletesubmission.php'>";
