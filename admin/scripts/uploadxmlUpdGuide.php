...
<!--Include custom CSS style sheets      -->
    ...
    $(this).parent('li').remove(); });
         ...
         <div  class='DivOne' style="background-color:#09F">
				<div class='myHeda' >
					Edit xml content & guide web pages
				</div>
			
			<div data-role="navbar">
				<table class="tbNav"><tr>
					<td><a href="../homeExhibit.php" class="tbNav">Home</a></td>
         ...
         <div  data-role='content' style="width:85%; background-color:#FFF; margin:0 auto" >
				...
				//echo '../xmlpages/'.$_POST['editSelectedGuide'].'.xml <br><hr>';
				
				//<a href="../xmlpages/hawreGuide2014031701.xml">hawreGuide2014031701.xml</a>
				$xmlfile= $_SESSION["xmlfile"];//$_COOKIE["TestCookie"];
				echo "cookee".$xmlfile;
				//echo"<input type='hidden' name='fileName' value='".$_POST['editSelectedGuide']."'>";
		   	   if (file_exists($xmlfile)) 
			   {       
			         $xml = simplexml_load_file('../xmlpages/'.$xmlfile);
				} else {   
			...
			if($child->getName() == 'Page')   
			 { 	
				foreach($child->attributes() as $key => $value) 
				 { 	
				  $pageNum = $value;
				echo"<ul><li><fieldset><legend>Page:".$value."</legend><input type='hidden' name='pageBeginning".$pageNum."'>"; 			
			  ...
			  { 
			   echo"<li>"; 
			   //parse each attribute under page element
			  ...
			  if($key =='type') 	
					...
					echo  "Question With Text Response (Single Line):<input type='text' value='". $subchild ."' name='QuestionType1". $count."'><br/>"; 
					$count++;
					}
					else if( $value=='Question Type2'){
					echo  "Question With Text Response (Multiple Line):<input type='text' value='". $subchild ."' name='QuestionType2". $count."'><br/>";
					$count++; 
					}
					else if( $value=='Question Type3'){
					echo  "Question With Numeric Response:<input type='text' value='". $subchild ."' name='QuestionType3". $count."'><br/>"; 
					$count++;
					}
					else if( $value=='Question Type4'){
			        echo"Question With Multiple Options (Single Choice):<input type='text' value='". $subchild ."' name='QuestionType4". $count."'><br/>"; 
					$count++;
					}
					else if( $value=='Question Type5'){
					echo"Question With Multiple Options (Multiple Choice):<input type='text' value='". $subchild ."' name='QuestionType5". $count."'><br/>"; 
					$count++;
					}
					else if( $value=='Question Type6'){
					echo  "Question With Image Upload Response:<input type='text' value='". $subchild ."' name='QuestionType6". $count."'><br/>"; 
					$count++;
					}
				} 	
			else {
				//read the attributes one by one and give them an input text for user to change the content.  
				$count=$count-1; 
